namespace A51C.Dens
{
    public class FileReadHelper : EnDecryptHelper
    {
        #region Simple
        ...
        /// <returns></returns>
        public static byte[] DecryptFormFilePath(string filePath, string companyname)
        {
            #region 解密文件
            // Creat -> FileInfo,
            var fileInfo = new System.IO.FileInfo(filePath);
            // Creat -> Key && IV
            var enckey = StringMd5ShaToString(false,
                                                              string.Format("<{0}/>{1}</{2}>", companyname,
                                                                            System.IO.Path.GetFileNameWithoutExtension(
                                                                                fileInfo.FullName), companyname), 16,
                                                              false, 1, System.Text.Encoding.UTF8);
            var enciv = StringMd5ShaToString(false,
                                                             string.Format("[{0}/]{1}[/{2}]", companyname,
                                                                           System.IO.Path.GetFileNameWithoutExtension(
                                                                               fileInfo.FullName), companyname), 16,
                                                             false, 1, System.Text.Encoding.UTF8);
            ...
            try
            {
                fileBytes = ByteAesToByte(true, System.IO.File.ReadAllBytes(filePath), enckey, enciv);
                if (fileBytes.IsEmptyBytes())
                {
                    try
                    {
        ...
        /// <param name="filePath"></param>
        /// <param name="companyname"></param>
        public static void EncryptFromFilePath(string filePath, string companyname)
        {
            #region 加密文件
            // Creat -> FileInfo,
            var fileInfo = new System.IO.FileInfo(filePath);
            ...
            try
            {
                fileBytes = ByteAesToByte(false, System.IO.File.ReadAllBytes(filePath), enckey, enciv);
            ...
            if (fileBytes.IsEmptyBytes()) return;
        ...
        public static System.Drawing.Image DecryptImageFormFilePath(string imagePath, string companyname)
        {
            ...
            try
            {
                var imgByte = ByteAesToByte(true, System.IO.File.ReadAllBytes(imagePath), enckey, enciv);
        ...
        public static string DecryptXmlFromFilePath(string xmlPath, string companyname)
        {
            ...
            try
            {
                decresult = ByteAesDecToString(System.IO.File.ReadAllBytes(xmlPath), enckey, enciv);
                                                ...
                                                string companyname)
        {
            ...
            var decresult = isImage
                                ? ByteAesToByte(false, System.IO.File.ReadAllBytes(fileInfo.FullName),
                                                                enckey, enciv)
                                : StringAesEncToByte(System.IO.File.ReadAllText(fileInfo.FullName),
                                                                     enckey, enciv);
