namespace A51C.Control.Mase
{
    public class ResxHelper
    {
        ...
        public System.Drawing.Image GetImage(string imageResxName)
        {
            ...
            {
                var stream = GetType().Assembly.GetManifestResourceStream("Geno.Control.Mase.Static." + imageResxName);
                if (stream == null)
                {
                    return null;
                }
        ...
        public System.Drawing.Icon GetIco(string icoResxName)
        {
            ...
            using (var stream = GetType().Assembly.GetManifestResourceStream("Geno.Control.Mase.Static." + icoResxName))
            {
                if (stream == null)
                {
                    return null;
