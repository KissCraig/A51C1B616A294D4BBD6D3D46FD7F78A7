namespace App.Resx
{
    public class ResxHelper
    {
        ...
        public System.Drawing.Image GetImage(string imageResxName)
        {
            ...
            {
                case 3:
                    localPath = string.Format("{0}\\Image\\{1}\\{2}.{3}", BasePublic.DataFolderDirectory, imageResxNames[0],
            ...
            {
                var stream = GetType().Assembly.GetManifestResourceStream("App.Resx.Image." + imageResxName);
                if (stream == null)
                {
                    return null;
                }
