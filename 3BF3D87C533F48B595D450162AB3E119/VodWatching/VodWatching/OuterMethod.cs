...
namespace VodWatching
{
    public class OuterMethod
    {
        ...
        {
            return (Size)Base.OwnerParent.GetType().GetField("_normalSize", BindingFlags.NonPublic | BindingFlags.Instance).GetValue(Base.OwnerParent);
        }
        ...
        {
            Base.OwnerParent.GetType().GetMethod("IWantUpdate", BindingFlags.Public | BindingFlags.Instance).Invoke(Base.OwnerParent, new object[] { Base.Guid, version, downloadPath });
        }
